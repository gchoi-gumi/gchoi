  ...
  const fetchRoutes = async () => {
    try {
      ...
      for (const query of searchQueries) {
        const promise = fetch(
          `https://${projectId}.supabase.co/functions/v1/make-server-a8dd3f70/search-places`,
          {
            method: "POST",
            headers: {
    ...
    <div className="w-full">
      <div className="bg-gradient-to-r from-indigo-600 to-indigo-800 py-12 mb-8">
        <div className="max-w-6xl mx-auto px-6">
          <button onClick={onBack} className="mb-6 flex items-center text-white/90 hover:text-white transition-colors">
            <ChevronLeft className="w-5 h-5" />
            <span>추천 정보</span>
          </button>

          <h1 className="text-white mb-2">추천 여행 경로</h1>
          <p className="text-gray-200">{travelStyle}형 맞춤 코스</p>
        </div>
      </div>
