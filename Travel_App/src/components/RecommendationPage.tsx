...
import { ArrowLeft, MapPin, Loader2, Save, Sparkles } from "lucide-react";
...
export function RecommendationPage(props: RecommendationPageProps) {
  const { travelStyle, location, accessToken, onBack, onShowMap, onShowRoutes, onShowSmartRoute, onSaveItinerary, onRetakeSurvey } = props;
  ...
  const fetchWeatherAndRecommendation = async () => {
    try {
      ...
      try {
        ...
        const weatherUrl = `https://${projectId}.supabase.co/functions/v1/make-server-a8dd3f70/weather/${encodeURIComponent(location)}`;
          ...
          if (weatherData && !weatherData.error) {
            ...
            if (weatherData.isMock) {
              if (weatherData.error === 'invalid_api_key') {
                ...
                console.warn("[RecommendationPage] Using mock weather data (OPENWEATHER_API_KEY may not be set)");
              }
            } else {
  ...
  const getTravelStyleIcon = (style: string) => {
    if (style === "힐링") return "🧘";
    if (style === "관광") return "🏛️";
    if (style === "액티비티") return "🏃";
    return "✈️";
  };

  const getTravelStyleColor = (style: string) => {
    if (style === "힐링") return "bg-gray-100 text-gray-800 border-gray-300";
    if (style === "관광") return "bg-gray-200 text-gray-900 border-gray-400";
    if (style === "액티비티") return "bg-gray-300 text-gray-900 border-gray-500";
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-indigo-800 py-12 mb-8">
        <div className="max-w-6xl mx-auto px-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-white">맞춤형 여행 추천</h1>
              <p className="text-gray-300 mt-1">{location} 여행</p>
            </div>
          </div>
        </div>
      </div>
