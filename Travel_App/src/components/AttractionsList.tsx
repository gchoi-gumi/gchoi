...
export function AttractionsList({ location, accessToken, weather, onBookmark }: AttractionsListProps) {
  ...
  const fetchAttractions = async () => {
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-a8dd3f70/attractions/${areaCode}`,
        {
          headers: {
      ...
      console.log('[AttractionsList] Received data:', {
        total: data.attractions?.length || 0,
        popular: data.popularPlaces?.length || 0,
        hidden: data.hiddenGems?.length || 0
      });
  ...
  const handleBookmarkClick = async (attraction: Attraction) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-a8dd3f70/bookmark`,
        {
          method: "POST",
          headers: {
