...
export function RouteMapPage({ places, routeInfo, transportMode, onBack }: RouteMapPageProps) {
  ...
  const drawRouteOnMap = () => {
    ...
    // Draw polyline connecting places with gradient effect
    if (pathCoords.length > 1) {
      // Create multiple polylines for gradient effect
      for (let i = 0; i < pathCoords.length - 1; i++) {
        const segmentPath = [pathCoords[i], pathCoords[i + 1]];
  ...
  const getPlaceImage = (placeId: string, category: string): string => {
    // First try to get image from place data (Google Places API)
    const googleImage = placeImages.get(placeId);
    if (googleImage) {
      return googleImage;
    }
  ...
  return (
    <div className="bg-gradient-to-br from-cyan-50 via-blue-50 to-teal-50 min-h-screen pb-20">
      {/* Header */}
      <div className="pt-6 px-6 pb-4 glass-cyan border-b border-cyan-100 sticky top-0 z-40">
        <button onClick={onBack} className="mb-4 flex items-center text-cyan-700 hover:text-cyan-900">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl mb-1 text-gray-900">경로 안내</h1>
            <p className="text-sm text-cyan-700">
              {getTransportIcon(transportMode)} {transportMode === "WALK" ? "도보" : transportMode === "TRANSIT" ? "대중교통" : "자동차"}
            </p>
          </div>
          <Badge className="gradient-ocean text-white shadow-lg">{places.length}개 장소</Badge>
        </div>
      </div>
      ...
      >
        ...
        {mapError && (
          <div className="absolute inset-0 flex items-center justify-center p-6">
            <div className="text-center glass p-6 rounded-2xl shadow-premium max-w-sm">
              <MapPin className="w-12 h-12 text-cyan-500 mx-auto mb-3" />
              <h3 className="mb-2 text-gray-900">지도를 표시할 수 없습니다</h3>
      ...
      {/* Route Summary */}
      {routeInfo && (
        <div className="px-6 py-4 glass-cyan border-y border-cyan-100">
          <div className="flex items-center justify-center gap-8">
            <div className="text-center">
              <div className="text-xs text-gray-600 mb-1">총 거리</div>
              <div className="text-sm text-cyan-700">{routeInfo.totalDistanceText || '계산 중...'}</div>
            </div>
            <div className="w-px h-8 bg-cyan-200"></div>
            <div className="text-center">
              <div className="text-xs text-gray-600 mb-1">이동 시간</div>
              <div className="text-sm text-cyan-700">{routeInfo.totalTimeText || '계산 중...'}</div>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="space-y-3">
          {places.map((place, index) => (
            <Card 
              key={place.id}
              id={`place-${index}`}
              className={`p-4 cursor-pointer transition-all glass hover-lift ${
            ...
            >
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {/* Route segment info */}
                  {index < places.length - 1 && routeInfo && routeInfo.routes && routeInfo.routes[index] && (
