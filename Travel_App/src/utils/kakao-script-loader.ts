// Kakao Maps SDK URL and App Key
const KAKAO_MAP_SDK_URL = 'https://dapi.kakao.com/v2/maps/sdk.js';
const KAKAO_APP_KEY = '94e86b9b6ddf71039ab09c9902d2d79f'; // JavaScript key
...
export function loadKakaoMapScript(): Promise<void> {
  ...
  if (isLoaded && window.kakao && window.kakao.maps) {
    console.log('[KakaoScript] ✅ Already loaded');
    return Promise.resolve();
  }

  // Currently loading - return existing promise
  if (isLoading && loadPromise) {
    console.log('[KakaoScript] ⏳ Loading in progress...');
    return loadPromise;
  }

  // Script already added to DOM - don't add again!
  if (scriptAdded || document.querySelector('script[src*="dapi.kakao.com"]')) {
    console.log('[KakaoScript] 📜 Script already in DOM');
    
    // Check if kakao exists now
    if (window.kakao && window.kakao.maps) {
      isLoaded = true;
      scriptAdded = true;
      console.log('[KakaoScript] ✅ SDK ready');
      return Promise.resolve();
    }
    
    // Script exists but not working - silently fail (REST API fallback will be used)
    scriptAdded = true;
    // Only log warning in localhost, not in Figma Make preview
    if (window.location.hostname === 'localhost') {
      console.warn('[KakaoScript] ⚠️ Script in DOM but SDK not loading');
    }
    return Promise.reject(new Error('Kakao Maps SDK script in DOM but not loading'));
  }

  console.log('[KakaoScript] 🚀 Loading SDK from:', KAKAO_MAP_SDK_URL);
  console.log('[KakaoScript] 🔑 App Key:', KAKAO_APP_KEY.substring(0, 8) + '...');
  console.log('[KakaoScript] 🌐 Current domain:', window.location.origin);
      ...
      if (window.kakao && window.kakao.maps) {
        ...
        try {
          window.kakao.maps.load(() => {
            ...
            console.log('[KakaoScript] ✅ SDK loaded and ready');
            resolve();
          });
        } catch (error) {
          isLoading = false;
          // Silent fail - REST API fallback will be used
          reject(error);
        }
      } else {
        // Silent fail - REST API fallback will be used automatically
        isLoading = false;
